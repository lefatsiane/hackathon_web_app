...
export default function ApplicationsScreen() {
  ...
  return <SafeAreaView style={styles.safe}><FlatList data={items} keyExtractor={(item) => item.id} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={colors.blue} />} contentContainerStyle={styles.page} ListHeaderComponent={<><Text style={styles.eyebrow}>{role === 'employer' ? 'APPLICANTS' : 'APPLICATIONS'}</Text><Text style={styles.title}>{role === 'student' ? 'Companies you applied to' : 'All applicants'}</Text><Text style={styles.muted}>{role === 'student' ? 'Track the opportunities and companies receiving your applications.' : 'Review applicants across all your opportunities.'}</Text></>} ListEmptyComponent={<View style={styles.empty}>{message ? <><ActivityIndicator color={colors.blue} /><Text style={styles.muted}>{message}</Text></> : <Text style={styles.muted}>No applications yet.</Text>}</View>} renderItem={({ item }) => <View style={styles.card}>{role === 'employer' ? <Pressable onPress={() => item.students?.id && router.push({ pathname: '/candidate/[id]', params: { id: item.students.id } })}><Text style={styles.heading}>{item.students?.full_name || 'Candidate'}</Text><Text style={styles.link}>View candidate details →</Text></Pressable> : <Text style={styles.heading}>{item.opportunities?.employers?.company_name || 'Employer'}</Text>}<Text style={styles.muted}>{item.opportunities?.title || 'Opportunity'}</Text><Text style={styles.status}>{item.status}</Text>{role === 'employer' && <View style={styles.statuses}>{statuses.map((status) => <Pressable key={status} disabled={busyId === item.id} onPress={() => changeStatus(item, status)} style={[styles.statusButton, item.status === status && styles.statusButtonActive]}><Text style={[styles.statusButtonText, item.status === status && styles.statusButtonTextActive]}>{status}</Text></Pressable>)}</View>}</View>} /></SafeAreaView>;
}

const makeStyles = (colors: ReturnType<typeof useTheme>['colors']) => StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, page: { padding: 20, gap: 14 }, back: { color: colors.muted, fontWeight: '700' }, eyebrow: { color: colors.blue, fontSize: 11, fontWeight: '800' }, title: { color: colors.text, fontSize: 29, lineHeight: 35, fontWeight: '800' }, muted: { color: colors.muted, lineHeight: 21 }, card: { gap: 8, padding: 18, borderColor: colors.border, borderWidth: 1, borderRadius: 14, backgroundColor: colors.surfaceRaised }, heading: { color: colors.text, fontSize: 17, fontWeight: '800' }, link: { color: colors.blue, fontWeight: '800' }, status: { color: colors.blue, fontWeight: '800', textTransform: 'capitalize' }, statuses: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 5 }, statusButton: { borderColor: colors.border, borderWidth: 1, borderRadius: 7, paddingHorizontal: 9, paddingVertical: 8 }, statusButtonActive: { borderColor: colors.blue, backgroundColor: colors.blueDark }, statusButtonText: { color: colors.muted, fontSize: 11 }, statusButtonTextActive: { color: colors.text, fontWeight: '800' }, empty: { alignItems: 'center', paddingVertical: 50, gap: 12 } });
